...
class NatoScraper(DocumentScraper):
    ...
    def __init__(self):
        super().__init__()
        self.fonte = "nato"
        self.url_base = self.config['fonti_dati']['nato']['url_base']
        self.document_urls = self.config['fonti_dati']['nato'].get('document_urls', [])
        self.sections = self.config['fonti_dati']['nato'].get('sections', [])
        
        # Pattern per l'estrazione dei dati
        self.patterns = {
            'nome_missione': r'(?:Operation|Mission)\s+([A-Za-z\s\-]+)',
            'paese': r'(?:in|at|located in)\s+([A-Za-z\s\-]+)',
            'data_inizio': r'(?:started|began|since)\s+(?:on)?\s*(\d{1,2}/\d{1,2}/\d{4})',
            'data_fine': r'(?:ended|until)\s+(?:on)?\s*(\d{1,2}/\d{1,2}/\d{4})',
            'personale_totale': r'(?:personnel|troops|forces)\s*(?:total)?\s*:\s*(\d+)',
            'costo_totale': r'(?:cost|budget|expenditure)\s*(?:total)?\s*:\s*€\s*([\d.,]+)',
            'tipo_missione': r'(?:type|nature)\s*(?:of operation)?\s*:\s*([A-Za-z\s\-]+)',
            'mandato': r'(?:mandate|resolution)\s*(?:NATO)?\s*:\s*([A-Za-z0-9\s\-]+)'
        }

    def estrai_dati(self) -> pd.DataFrame:
        """Estrae i dati dalle pagine della NATO"""
        self.logger.info("Inizio estrazione dati dalla NATO")
        dati = []
        ...
        if not dati:
            self.logger.error("Nessun dato estratto dalla NATO")
            return pd.DataFrame()
            
        df = pd.DataFrame(dati)
        self._salva_dati_raw(dati, "nato_raw")
        df = self.pulisci_dati(df)
        if self.valida_dati(df):
            self._salva_dati_processati(df, "nato_processed")
            return df
        else:
    ...
    def _estrai_dati_missione(self, missione: BeautifulSoup) -> Dict:
        ...
        try:
            ...
            if date:
                ...
                data_inizio = re.search(r'(?:started|began|since)\s+(?:on)?\s*(\d{1,2}/\d{1,2}/\d{4})', date_text)
                data_fine = re.search(r'(?:ended|until)\s+(?:on)?\s*(\d{1,2}/\d{1,2}/\d{4})', date_text)
                if data_inizio:
