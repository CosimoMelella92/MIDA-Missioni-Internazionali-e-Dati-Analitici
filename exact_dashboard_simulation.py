#!/usr/bin/env python3
"""
Script per simulare esattamente la logica della dashboard usando solo il nome della missione
"""
...
def exact_dashboard_simulation():
    """Simula esattamente la logica della dashboard"""
    
    print("🔍 Simulazione esatta della dashboard")
    print("=" * 50)
    ...
    # 5. Simula la deduplicazione della dashboard (SOLO per nome)
    df_combined['nome_normalizzato'] = df_combined['nome'].apply(normalize_name)
    
    # Trova duplicati basati SOLO sul nome normalizzato
    duplicates = df_combined[df_combined.duplicated(subset=['nome_normalizzato'], keep=False)]
    
    if len(duplicates) > 0:
        print(f"\n🔍 Duplicati trovati ({len(duplicates)} righe):")
        for _, group in duplicates.groupby(['nome_normalizzato']):
            print(f"\n📋 Gruppo duplicato per nome '{group.iloc[0]['nome']}':")
            for _, row in group.iterrows():
                print(f"   - {row['nome']} ({row['paese']}) - {row['tipo_missione']} - Fonte: {row['fonte']}")
    
    # 6. Rimuovi duplicati (mantieni il primo) - SOLO per nome
    df_final = df_combined.drop_duplicates(subset=['nome_normalizzato'], keep='first')
...
if __name__ == "__main__":
    removed = exact_dashboard_simulation() 